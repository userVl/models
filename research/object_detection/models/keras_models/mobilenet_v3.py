...
"""A wrapper around the MobileNet v3 models for Keras, for object detection."""
...
class _LayersOverride(object):
  """Alternative Keras layers interface for the Keras MobileNetV3."""
  ...
  def __init__(self,
               ...
               min_depth=None,
               conv_defs=None):
    """Alternative tf.keras.layers interface, for use by the Keras MobileNetV3.

    It is used by the Keras applications kwargs injection API to
    modify the Mobilenet v3 Keras application with changes required by
    ...
    Args:
      ...
      conv_hyperparams: A `hyperparams_builder.KerasLayerHyperparams` object
        ...
        to use default mobilenet_v3 layer builders.
  ...
  def Conv2D(self, filters, **kwargs):
    ...
    Overrides the Keras MobileNetV3 application's convolutions with ones that
...
def mobilenet_v3_small(batchnorm_training,
                 ...
                 conv_defs=None,
                 **kwargs):
  """Instantiates the MobileNetV3Small architecture, modified for object detection.

  This wraps the MobileNetV3Small tensorflow Keras application, but uses the
...
def mobilenet_v3_large(batchnorm_training,
                 ...
                 conv_defs=None,
                 **kwargs):
  """Instantiates the MobileNetV3Large architecture, modified for object detection.

  This wraps the MobileNetV3Large tensorflow Keras application, but uses the
  Keras application's kwargs-based monkey-patching API to override the Keras
  architecture with the following changes:
  ...
  layers_override = _LayersOverride(
      ...
      alpha=alpha,
      conv_defs=conv_defs)
  return tf.keras.applications.MobileNetV3Large(alpha=alpha,
                                           layers=layers_override,
                                           **kwargs)
# pylint: enable=invalid-name
